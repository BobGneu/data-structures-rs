...
pub fn swap_arr(array: &mut [i32],from: usize,to: usize) { 
    let tmp = array[from];
    array[from] = array[to];
    array[to] = tmp;
}

pub fn bubble_sort_arr(array: &mut [i32]) {
    ...
    loop {
        ...
        loop {
            ...
            if array[walker] > array[walker + 1] {
                swap_arr(array, walker, walker + 1);
            }
...
pub fn selection_sort_arr(array: &mut [i32]) {
    let mut number_of_sorted_elements: usize = 0;
    let mut smallest: usize = 0;
    let mut walker: usize = number_of_sorted_elements + 1;
    ...
    loop {
        ...
        if smallest != number_of_sorted_elements {
            swap_arr(array, smallest, number_of_sorted_elements);
        }
